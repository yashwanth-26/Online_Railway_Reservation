...
export default function AddTrains() {
    ...
    const handleadd = () => {
		...
		console.log(berthDetailsModel[0].cost);
        let obj = { id: Math.floor((Math.random() * 100) + 1), trainNumber: trainNumber, trainName: trainName,  trainFrom:trainFrom,
             trainTo:trainTo, trainAvaliablity:trainAvaliablity,sleeperCost:sleeperCost,acCost:acCost,
   ...
   <div>
       ...
       <h1 className="h1tag">Add Train</h1>
	<form>
		...
		<div class="row align-items-start">
			...
			<div class="col">
				<label for="fname">Sleeper Cost:</label>
				<input type="number" id="triannumber" name="fname" placeholder="Cost in dollars..." onChange={(event) => setSleeperCost(event.target.value)}/><br/><br/>
			</div>
			<div class="col">
				<label for="fname">AC Cost:</label>
				<input type="number" id="triannumber" name="fname" placeholder="cost in dollars..." onChange={(event) => setAcCost(event.target.value)}/><br/><br/>
			</div>
